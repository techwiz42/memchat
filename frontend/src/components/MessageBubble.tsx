  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-3 group`}>
      <div className={`relative max-w-[75%]`}>
          ...
          {editing ? (
            <div className="space-y-2">
              <textarea
                ...
                rows={3}
                className="w-full rounded-lg border border-blue-300 px-2 py-1.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 resize-vertical"
                autoFocus
              />
              <div className="flex gap-2 justify-end">
                <button
                  onClick={() => setEditing(false)}
                  className="px-2.5 py-1 text-xs rounded bg-gray-200 text-gray-700 hover:bg-gray-300"
                >
                  Cancel
                </button>
                <button
        ...
        {/* Edit / Regenerate hover buttons */}
        {!editing && (
          <div className={`absolute top-0 ${isUser ? "left-0 -translate-x-full pr-1" : "right-0 translate-x-full pl-1"} opacity-0 group-hover:opacity-100 transition-opacity flex gap-0.5`}>
            {isUser && onEdit && (
              <button
                onClick={() => { setEditText(message.content); setEditing(true); }}
                className="p-1 rounded text-gray-400 hover:text-blue-600 hover:bg-gray-100"
                title="Edit message"
              >
